...
def _unzip_office_doc(filename):
    ...
    try:
        file_type = subprocess.check_output(["file", filename]).decode("ascii")
...
def detect_stomping_via_pcode(filename, verbose=False, sensitivity="medium"):
    ...
    pcode = None
    try:
        pcode = subprocess.check_output(["python", os.path.join(os.environ["PCODEDMP_DIR"], "pcodedmp.py"), filename]).decode("ascii")
    except Exception as e:
    ...
    vba = None
    try:
        vba = subprocess.check_output(["olevba", filename]).decode("ascii")
    except Exception as e:
